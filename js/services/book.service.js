...
function addFeddback(id, feedback) {
  booksDB = booksDB.map(book => {
    if (book.id === id) {
      if (!book.feedback) book.feedback = [feedback]
      else book.feedback.push(feedback)
    }
    return book;
  });
...
function deleteReview(bookId, revId) {
  booksDB = booksDB.map(book => {
    if (book.id === bookId) {
      book.feedback.map((review, idx) => {
        if (review.id === revId) book.feedback.splice(idx, 1);
        return review;
      });
    }
    return book;
...
function findBook(searchKey) {
  return axios.get(`https://www.googleapis.com/books/v1/volumes?printType=books&q=effective%20${searchKey}`)
    .then(books => {
      return books.data.items.map(book => {
        var bookVal = _checkValue(book);
        return {
          id: book.id,
          title: book.volumeInfo.title,
          thumbnail: bookVal.thumbnail,
          authors: book.volumeInfo.authors,
          publishedDate: book.volumeInfo.publishedDate,
          listPrice: bookVal.listPrice,
    ...
    .catch(() => console.log('Goggle is S#I!'));
}
...
function neighborBook(bookId) {
  const bookIdx = booksDB.findIndex(book => book.id === bookId);
  const book = {
    next: (booksDB[bookIdx + 1]) ? booksDB[bookIdx + 1] : booksDB[0],
    prev: (booksDB[bookIdx - 1]) ? booksDB[bookIdx - 1] : booksDB[booksDB.length - 1]
  };
  return Promise.resolve(book)
}

function _checkValue(book) {
  var thumbnail;
  var listPrice;
